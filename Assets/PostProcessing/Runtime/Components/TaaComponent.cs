    ...
    public sealed class TaaComponent : PostProcessingComponentRenderTexture<AntialiasingModel>
    {
        ...
        Vector2 GenerateRandomOffset()
        {
            var offset = new Vector2(
                this.GetHaltonValue(this.m_SampleIndex & 1023, 2), this.GetHaltonValue(this.m_SampleIndex & 1023, 3));
        ...
        {
            var vertical = Mathf.Tan(0.5f * Mathf.Deg2Rad * this.context.camera.fieldOfView);
            var horizontal = vertical * this.context.camera.aspect;

            offset.x *= horizontal / (0.5f * this.context.width);
            offset.y *= vertical / (0.5f * this.context.height);

            var left = (offset.x - horizontal) * this.context.camera.nearClipPlane;
            var right = (offset.x + horizontal) * this.context.camera.nearClipPlane;
            var top = (offset.y + vertical) * this.context.camera.nearClipPlane;
            var bottom = (offset.y - vertical) * this.context.camera.nearClipPlane;
        ...
        {
            var vertical = this.context.camera.orthographicSize;
            var horizontal = vertical * this.context.camera.aspect;
