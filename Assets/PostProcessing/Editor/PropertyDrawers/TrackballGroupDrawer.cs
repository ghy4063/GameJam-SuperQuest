    ...
    sealed class TrackballGroupDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            while (enumerator.MoveNext())
            {
                ...
                this.OnWheelGUI(position, size, prop.Copy());
        ...
        void OnWheelGUI(Rect position, int size, SerializedProperty property)
        {
            ...
            hsv = this.GetInput(bounds, hsv, radius);
            ...
            if (this.TryGetDisplayValue(value, property, out data))
            {
                position.x = oldX;
            ...
            if (this.m_ResetState)
            {
                value = Color.clear;
				this.m_ResetState = false;
            }
        ...
        Vector3 GetInput(Rect bounds, Vector3 hsv, float radius)
        {
            ...
            if (e.type == EventType.MouseDown && GUIUtility.hotControl == 0 && bounds.Contains(mousePos))
            {
                if (e.button == 0)
                {
                    ...
                    {
                        e.Use();
                        this.GetWheelHueSaturation(relativePos.x, relativePos.y, radius, out hsv.x, out hsv.y);
